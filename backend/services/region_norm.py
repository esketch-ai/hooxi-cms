...
- 2단계: ①명칭 표 ②접미사 제거 폴백 — '서울특별자치시'·'전남광주통합특별시' 같은
  실존하지 않는 변형 표기도 접미사를 벗겨 알려진 지역으로 귀결될 때만 치환한다.
- '전남광주'(버스조합 통합 관용 표기)는 공통코드 정합을 위해 **전남**으로 귀결(사용자 확정).
- 멱등: 이미 단축형이거나 귀결 불가 값(고속 등)은 그대로 보존.
"""
...
REGION_FULL_TO_CODE = {
    ...
    "경상남도": "경남",
    "제주특별자치도": "제주", "제주도": "제주",
    # 버스조합 통합 관용 표기 — 공통코드(REGION)에 없는 값이라 전남으로 귀결(사용자 확정)
    "전남광주": "전남",
}


# 접미사 폴백의 귀결 허용치 — 공통코드 REGION 단축형 17종
KNOWN_REGIONS = set(REGION_FULL_TO_CODE.values())
...
def normalize_region(raw) -> str:
    ...
    예: '경상남도'→'경남'(표), '전남광주'→'전남'(표), '서울특별자치시'→'서울',
    '전남광주통합특별시'→'전남'(폴백). 귀결 불가('고속' 등)는 그대로 반환(멱등)."""
    s = str(raw or "").strip()
    key = s.replace(" ", "")
