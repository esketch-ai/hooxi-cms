...
def test_normalize_region_idempotent_and_preserves_legacy():
    # 이미 단축형·귀결 불가 관용 표기는 그대로(멱등)
    for v in ["서울", "경기", "고속", ""]:
        assert normalize_region(v) == v
    # 전남광주(버스조합 통합 표기)는 공통코드 정합을 위해 전남으로(사용자 확정)
    assert normalize_region("전남광주") == "전남"
...
def test_ensure_schema_normalizes_existing_regions(client):
    ...
    try:
        ...
        db.add(models.Client(
            client_type="TRANSPORT", company_name="지역보정변형운수", region="전남광주",
        ))
        db.commit()
