...
KNOWN_DEFAULTS = {
    ...
    "company_biz_reg_no": (
        "",
        "자사 사업자등록번호(복수 가능, 콤마 구분) — 세금계산서 HTML 자동반영의 복호화 비밀번호 후보 및 "
        "매입/매출 방향 판정 기준. 예: 529-81-02298, 000-00-00000 (후시파트너스·후시제주랩 등 전부)",
    ),
    "tax_invoice_dropbox_folder": (
        "",
        "세금계산서 HTML 스캔 기본 Dropbox 폴더(하위 포함) — 비우면 스캔 시 폴더를 직접 지정. "
        "예: /세금계산서 (앱 폴더 기준 상대경로)",
    ),
}
