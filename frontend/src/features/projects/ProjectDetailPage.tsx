    ...
    return (
      <div className="flex items-center gap-1.5">
        <input
          ...
          autoFocus
          className="h-8 w-32 rounded-lg border border-slate-300 px-2 text-sm focus:border-slate-500 focus:outline-none"
          placeholder="원/tCO₂"
          aria-label="배출권 단가"
        />
        <button
          ...
          className="rounded-md bg-slate-800 p-1.5 text-white hover:bg-slate-700 disabled:opacity-60"
          title="저장"
          aria-label="단가 저장"
        >
          {update.isPending ? (
  ...
  return (
    <div className="flex items-center gap-1.5">
      {unitPrice != null ? (
        ...
        <span className="font-medium text-amber-600">미정</span>
      )}
      <button
        type="button"
        onClick={startEdit}
  ...
  return (
    <div className="flex-1">
      <div className="flex items-center justify-between text-xs">
        <span className="font-medium text-slate-500">배분율 합계</span>
      ...
      <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-100">
        <div
          className={`h-full rounded-full transition-all ${over ? 'bg-rose-500' : 'bg-slate-800'}`}
...
export function ProjectDetailPage() {
  ...
  const mappingColumns: Column<ProjectClientMap>[] = [
    ...
    },
    {
      ...
      render: (m) =>
        m.asset_id ? (
          <span className="text-xs text-slate-500">{m.asset_summary ?? m.asset_id}</span>
        ) : (
      ...
      render: (m) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <button
            ...
            className="rounded-md p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-30"
    ...
    <div className="animate-fade-in space-y-4">
      ...
      <PageHeader
        ...
        actions={
          ...
          <button
            ...
            className="hidden items-center gap-1.5 rounded-lg border border-slate-200 px-3.5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 sm:flex"
          >
            <PencilSimple size={15} />
            사업 수정
          </button>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-slate-400">참여 {mappings.length}개사</span>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {project.issued_credits != null && (
            <OverviewItem label="확정 발급량">
              ...
              />
              {project.issued_at && (
                <span className="ml-1.5 text-xs text-slate-400">({fmtDate(project.issued_at)})</span>
              )}
            </OverviewItem>
          )}
      ...
      <section className="space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <h2 className="text-base font-bold text-slate-900">참여 고객사 매핑</h2>
          {/* 매핑 편집 — 모바일 숨김 (§7.1) */}
          <button
            type="button"
            onClick={() => {
