...
import { CalcRunPanel } from './CalcRunPanel'
import { VehicleLogPanel } from './VehicleLogPanel'
...
export function RegistryPage() {
  ...
  const [tab, setTab] = useState<'vehicles' | 'finance' | 'charging' | 'logs' | 'verify' | 'calc'>('vehicles')
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex w-fit items-center gap-1 rounded-full border border-hairline bg-elevate p-0.5">
        {(
          [
            ...
            { key: 'logs', label: '운행·충전 로그' },
            { key: 'verify', label: '대체도입 검증' },
            { key: 'calc', label: '감축량 산정' },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
      ...
      {tab === 'logs' && <VehicleLogPanel />}
      {tab === 'verify' && <VerificationPanel />}
      {tab === 'calc' && <CalcRunPanel />}
      {tab === 'vehicles' && (
        <>
      {/* 요약 타일 — 클릭 시 role 필터 */}
